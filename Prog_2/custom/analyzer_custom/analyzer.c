...
void main(int argc,char **argv){
  ...
  u_int aux_ui;
  FILE *fp;
  tcp_flow=(struct tcp_stream_struct *)malloc(sizeof(struct tcp_stream_struct));
  memset(tcp_flow,0,sizeof(struct tcp_stream_struct));
  tcp_flow->sourceP=0;
  if(argc!=2){
  ...
  for(;;){
    ...
    if(strcmp(buffer,"tcp")==0){
      for(;;){
        ...
        if(strcmp(buffer,"stream")==0)s_tcp=1;
        if(strcmp(buffer,"filt")==0){
          aux1_tcp=filt_tcp;
          if(aux1_tcp!=NULL)
       ...
       if(strcmp(buffer,"follow")==0){  
          //tcp_flow=tcp_stream_struct;
          fscanf(fp,"%s",buffer);
          aux=strtok(buffer, ".");
          tcp_flow->sIP[0]=atoi(aux);
          for(i=1;i<4;i++){
            aux=strtok(NULL,".");
            tcp_flow->sIP[i]=atoi(aux);
          }
          /*fscanf(fp, "%s", buffer);
          tcp_flow->sourceP =atoi(buffer);*/
          fscanf(fp,"%s",buffer);

          aux=strtok(buffer, ".");
          tcp_flow->dIP[0]=atoi(aux);
          for(i=1;i<4;i++){
            aux=strtok(NULL,".");
            tcp_flow->dIP[i]=atoi(aux);
          }
          fscanf(fp, "%s", buffer);
          tcp_flow->destP=atoi(buffer);
        }
        /* ############################################*/
  ...
  if(pipe(fpipe)<0)exit(1);
  if((pid=fork())>0){
    close(fpipe[1]);
    buffer_process(fpipe[0],tcp_flow->sIP,tcp_flow->dIP);
    close(fpipe[0]);
    return;
  }else if(pid<0)
    exit(-1);
  close(fpipe[0]);
  mem=fopen("log","wt");
  pd=pcap_open_live(device,LENSNIF,1,1000,buffer);
  if(pd==NULL)exit(1);

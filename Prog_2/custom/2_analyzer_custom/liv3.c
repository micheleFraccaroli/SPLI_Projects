...
void liv3(u_int type,const u_char *p){
  ...
  case 0x800:
    ...
    if(s_tcp)
      for(k=0; k<4; k++) {
        //printf("k=%d\n",k);
        //printf("%d %d\n",(int)(tcp_flow->sIP[k]),(int)(*sp));

        if(tcp_flow->sIP[k]==(*(sp+k)) || tcp_flow->sIP[k]==(*(dp+k)));
        else{
           tcp_flow->ip_flag=0;
           break;
        }
        if(tcp_flow->dIP[k]==(*(sp+k)) || tcp_flow->dIP[k]==(*(dp+k)));
        else{
           tcp_flow->ip_flag=0;
           break;
        }
        /*if(tcp_flow->sIP[k]!=(*(sp+k))){
          //printf("i'm in! :)\n");
          tcp_flow->ip_flag=0;
          break;
        }
        else if(tcp_flow->sIP[k]!=(*(dp+k))){
          tcp_flow->ip_flag=0;
          break;
        }

        if(tcp_flow->dIP[k]!=(*(dp+k))){
          tcp_flow->ip_flag=0;
          break;
        }
        else if(tcp_flow->dIP[k]!=(*(sp+k))){
          tcp_flow->ip_flag=0;
          break;
        }*/
      }
